...
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| `-a` | **(Required)** The architecture that the builder will run on. Creating an apk file for that specific architecture. See supported Architecures. | `-a amd64`                                                           |
...
| `-t` | **(Optional)** Set `true` to enable the testing repository.                                                                                    | `-t true`                                                            |

\*_If no private key is specified, one will be generated in the output directory._

Putting these all together in a command:

    ./build.sh -a amd64 -k /home/alex/key.rsa -i /home/alex/smbclient/ -o /home/alex/
...
I have not incorporated an easy way to do this in the scripts but running the commands separately doesn't hurt, here's an example to build for all supported architectures.
    ...
    #!/bin/bash
    for arch in amd64 armv6 armv7 armv8 i386 ppc64le s390x; do
    	./build.sh -a $arch -k <key> -i <input> -o <output>
    done
...
Currently all architectures are supported on Alpine 3.13, feel free to open an issue if you need a specific Alpine version!

| Platform       | Alpine Version | `-a` Input | Output Foldername |
| -------------- | -------------- | ---------- | ----------------- |
| linux/amd64    | 3.13           | `amd64`    | `x86_64`          |
| linux/arm/v6   | 3.13           | `armv6`    | `armhf`           |
| linux/arm/v7   | 3.13           | `armv7`    | `armv7`           |
| linux/arm64/v8 | 3.13           | `armv8`    | `aarch64`         |
| linux/386      | 3.13           | `i386`     | `x86`             |
| linux/ppc64le  | 3.13           | `ppc64le`  | `ppc64le`         |
| linux/s390x    | 3.13           | `s390x`    | `s390x`           |
